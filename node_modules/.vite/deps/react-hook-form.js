...
var getFieldArrayParentNames = (names, name) => {
	...
	const matches = [];
	let prefix = parts[0];
	for (let i = 1; i < parts.length; prefix += "." + parts[i++]) !isNaN(+parts[i]) && names.has(prefix) && matches.push(prefix);
	return matches;
};
...
function createFormControl(props = {}) {
	...
	const setValue = (name, value, options = {}) => {
		...
		if (!isValueUnchanged) {
			...
			if (!isFieldArray) for (const arrayName of getFieldArrayParentNames(_names.array, name)) _subjects.array.next({
				name: arrayName,
				values
			});
			_subjects.state.next({
	...
	const setValues = (formValues) => {
		...
		if (!deepEqual(_formValues, updatedFormValues)) {
			...
			for (const fieldName of _names.mount) setValue(fieldName, get(updatedFormValues, fieldName));
			_subjects.state.next({
				..._formState,
				values: _formValues
			});
